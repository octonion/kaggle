...
dbWriteTable(con,c("march_madness","w_parameter_levels"),parameter_levels,row.names=TRUE)
...
#model0 <- ps ~ year+field+d_conf+o_conf+(1|offense)+(1|defense)
...
dbWriteTable(con,c("march_madness","w_basic_factors"),as.data.frame(combined),row.names=TRUE)
...
jpeg("diagnostics/w_shapiro-francia.jpg")
...
jpeg("diagnostics/w_fitted_vs_residuals.jpg")
plot(f,r)
jpeg("diagnostics/w_q-q_plot.jpg")
