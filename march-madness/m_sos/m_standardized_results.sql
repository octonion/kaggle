...
drop table if exists march_madness.m_results;

create table march_madness.m_results (
	game_id		      integer,
	season		      integer,
	team_name	      text,
	team_id           integer,
...
from march_madness.m_regular_season_compact_results
union
select
1000000+game_id,
season,
...
from march_madness.m_team_conferences mtc
where (mtc.season,mtc.team_id)=(m_results.season,m_results.team_id);

update march_madness.m_results
set opponent_conf_id=conf_abbrev
from march_madness.m_team_conferences mtc
where (mtc.season,mtc.team_id)=(m_results.season,m_results.opponent_id);
